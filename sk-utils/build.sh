...
docker run -v $(pwd)/files:/opt/tak/certs/files \
    --name takserver --network takserver-net \
    -p 8089:8089 -p 8443:8443 -p 8444:8444 \
    -p 8446:8446 -t -d takserver-hardened

echo "Generating admin cert fingerprint"
FPrint=$(docker exec -it ca-setup bash -c "openssl x509 -noout -fingerprint -sha256 -inform pem -in files/admin.pem | grep -oP 'SHA256 Fingerprint=\K.*'")
...
if [ $? -ne 0 ]; then
  echo "Failed to install admin cert fingerprint to takserver" 
  echo "Loading admin cert"
  docker exec -it takserver bash -c 'java -jar /opt/tak/utils/UserManager.jar certmod -A /opt/tak/certs/files/admin.pem'
  if [ $? -ne 0 ]; then
    echo "Failed to load admin cert"
    exit 1
  fi
fi